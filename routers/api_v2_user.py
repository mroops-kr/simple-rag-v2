...
def upload(
    ...
    return cm_upload(userId, asyncYn, files)

@router.post('/reEmbedding', description="""
[사용자파일] 업로드된 파일을 다시 임베딩함
...
def reEmbedding(
    ...
    return cm_reEmbedding(userId, asyncYn, fileIds)

@router.post('/checkFinish', description="""
[사용자파일] asyncYn: Y 로 호출된 경우, 해당 파일의 완료여부 검사. 2분간 없으면 리턴함, 리턴값 확인 후 재호출 필요
...
def checkFinish(
    ...
    return cm_checkFinish(userId, fileIds, finishAnyYn)

@router.post('/search', description="""
[사용자파일] 임베딩된 백터DB에서 question(질문)과 유사도 검사후 결과 리턴함
...
def search(
    ...
    return cm_search(userId, question, n_results, fileIds)
...
def summary(
    ...
    return cm_summary(userId, fileIds)

@router.post('/delete', description="""
[사용자파일] 임베딩된 데이터 및 파일 삭제
...
def delete(
    ...
    return cm_delete(userId, fileIds)
...
def download(
    ...
    return cm_download(userId, fileId)
